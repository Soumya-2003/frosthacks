...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const startOfDay = new Date(today.setUTCHours(0, 0, 0, 0)); // Start of today (midnight)
    const endOfDay = new Date(today.setUTCHours(23, 59, 59, 999)); // End of today (just before midnight)

    // Fetch journal entries for the current day
    ...
    if (!journals || journals.length === 0) {
      return NextResponse.json({ message: "No journal entries found for today" }, { status: 404 });
    }
    ...
    // Process each journal entry for the day
    ...
    // Determine overall mood for the day
